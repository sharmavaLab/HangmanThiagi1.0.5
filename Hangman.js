...
function refreshImage(OuterDiv) {
	...
	elem.setAttribute("src", "images/"+Hdata.imageSet+"/hang_" + imgNo + ".gif");
	if (layout === "regular") {
...
function createPuzzles(OuterDiv, Question, Answer, gameProgress) {
	...
	createRefreshButton(OuterDiv);
	refreshImage(OuterDiv);

}

function createRefreshButton(OuterDiv) {
	var outerDivObj = document.getElementById(OuterDiv);

    var refreshButtonDiv = document.createElement("div");
    refreshButtonDiv.setAttribute("class","refreshButtonDiv");
	var refreshButton = document.createElement("button");
	refreshButton.setAttribute("class", "refreshButton");
	refreshButton.setAttribute("onclick", "refreshHangmanApp("+OuterDiv+")");
    refreshButton.innerHTML = "Refresh";

    refreshButtonDiv.appendChild(refreshButton)
	outerDivObj.appendChild(refreshButtonDiv);
}

function refreshHangmanApp(div_id){
	hangmanApp(div_id,map[div_id].questionString, map[div_id].answerString,map[div_id].count, map[div_id].layout, map[div_id].imageSet);
}

function hangmanApp(div_id, questions, answers, count, layout,imageSet) {
	...
	//TODO : Handling for default case missing for questions and answers
	setData.questionString = questions;
	setData.answerString = answers;


    //Imposing default value assignment for count(handeled for negative and undefined).
    ...
    //Imposing the default value for layout
    if(layout!=undefined){
	  setData.layout = layout;
	}

    //Imposing the default value for image set
    if(imageSet!=undefined){
       setData.imageSet = imageSet;
    }
